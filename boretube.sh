...
# ── TV config: load from tv.conf (generated by detect.sh) or use defaults ─
TV_IP="192.168.1.3"
DIAL_PORT=8008
CAST_PORT=8009
TV_NAME=""
if [[ -f "$SCRIPT_DIR/tv.conf" ]]; then
    # shellcheck source=/dev/null
    source "$SCRIPT_DIR/tv.conf"
fi
...
draw_banner() {
    clear
    local tv_label="${TV_NAME:-$TV_IP}"
    # Center the TV label inside a 43-char box
    local pad_total=$((43 - ${#tv_label}))
    ...
    echo "  ║${left_spaces}${tv_label}${right_spaces}║"
    echo "  ╚═══════════════════════════════════════════╝"
    echo -e "${RESET}"
}
