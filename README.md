...
```
boretube/
├── boretube.sh          # The entire tool — 797 lines of bash + embedded Python
├── whitelist.conf       # Allowed app IDs (one per line: ID|Name)
├── boretube.log          # Enforcement history (auto-created)
├── .venv/               # Python virtual environment
│   └── lib/python3.12/site-packages/
│       ├── protobuf/    # Google Protocol Buffers (for CastV2 wire format)
│       ├── PyChromecast/ # Installed but not used at runtime (discovery failed)
│       ├── zeroconf/    # Installed but not used at runtime (mDNS is link-local)
│       ├── casttube/    # Installed but not used at runtime
│       └── requests/    # Installed but not used at runtime
...
```bash
# Clone or copy boretube.sh to your machine
mkdir -p ~/boretube && cd ~/boretube
...
Edit the top of `boretube.sh` to match your network:

```bash
TV_IP="192.168.1.3"      # Your TV's IP address
DIAL_PORT=8008            # Standard, don't change unless your TV is special
CAST_PORT=8009            # Standard, don't change
LOCK_INTERVAL=10          # Seconds between checks (10 is the sweet spot)
```
